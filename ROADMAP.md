...
- C-部分. UFJ銀行/ニコスCSV取込(部分完了 2026-06-28)
  ...
  - 課題: 事業/プライベートが混在、後から判別不可
    - 解決策: 事業用カード分離 or レシート画像紐付け

### 着手順(最速完成戦略)

#### 1. レシート画像のGoogle Drive保管 + 取引紐付け(3〜5日) ★最優先
- 電子帳簿保存法対応(2024年義務化)
- 現状の課題: OCR後にレシート画像を破棄してる、税務上アウト
- 実装内容:
  - レシートOCR時に画像を accounting-app-data/receipts/ に保存
  - Transaction型に receiptUrl 追加
  - 取引一覧にレシート画像プレビューアイコン
  - CSV取込分にも後付けでレシート画像を紐付け可能に
  - 取引編集モーダルに画像アップロード/置換機能

#### 2. C-残: ニコス取込のUX改善(3〜5日)
